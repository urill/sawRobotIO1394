...
void osaIO1394XMLConfig::LoadFromFile(const std::string & filename, osaIO1394::Configuration & config)
{
    //CMN_LOG_RUN_VERBOSE << "Configuring from " << filename << std::endl;
    ...
    for (int i = 0; i < num_robots; i++) {
        ...
        if (osaIO1394XMLConfig::ConfigureRobot(xmlConfig, i + 1, robot)) {
    ...
    for (int i = 0; i < num_digital_inputs; i++) {
        osaIO1394::DigitalInputConfiguration digital_input;

        // Store the digital_input in the config if it's succesfully parsed
        if (osaIO1394XMLConfig::ConfigureDigitalInput(xmlConfig, i + 1, digital_input)) {
            config.DigitalInputs.push_back(digital_input);
        }
    }
}

bool osaIO1394XMLConfig::ConfigureRobot(cmnXMLPath & xmlConfig,
                                        ...
                                        osaIO1394::RobotConfiguration & robot)
{
    ...
    for (int i = 0; i < robot.NumberOfActuators; i++) {
        osaIO1394::ActuatorConfiguration actuator;
        ...
        if ((actuator.BoardID < 0) || (actuator.BoardID >= (int)osaIO1394::MAX_BOARDS)) {
        ...
        if ((actuator.AxisID < 0) || (actuator.AxisID >= (int)osaIO1394::MAX_AXES)) {
    ...
    if (xmlConfig.GetXMLValue(context, path, potentiometerPosition)) {
        if (potentiometerPosition == "Actuators") {
            robot.PotLocation = osaIO1394::POTENTIOMETER_ON_ACTUATORS;
        } else if (potentiometerPosition == "Joints") {
            robot.PotLocation = osaIO1394::POTENTIOMETER_ON_JOINTS;
        } else {
    ...
    // Configure Coupling
    if (!osaIO1394XMLConfig::ConfigureCoupling(xmlConfig, robotIndex, robot)) {
        return false;
    }
...
bool osaIO1394XMLConfig::ConfigureCoupling(cmnXMLPath & xmlConfig,
                                           const int robotIndex,
                                           osaIO1394::RobotConfiguration & robot)
{
    char path[64];
    ...
    if (robot.HasActuatorToJointCoupling) {
        ...
        parse_success &= ConfigureCouplingMatrix(xmlConfig, robotIndex, "ActuatorToJointPosition",
                                                 robot.NumberOfJoints, robot.NumberOfActuators,
                                                 robot.ActuatorToJointPosition);

        parse_success &= ConfigureCouplingMatrix(xmlConfig, robotIndex, "JointToActuatorPosition",
                                                 robot.NumberOfActuators, robot.NumberOfJoints,
                                                 robot.JointToActuatorPosition);

        parse_success &= ConfigureCouplingMatrix(xmlConfig, robotIndex, "ActuatorToJointTorque",
                                                 robot.NumberOfJoints, robot.NumberOfActuators,
                                                 robot.ActuatorToJointEffort);

        parse_success &= ConfigureCouplingMatrix(xmlConfig, robotIndex, "JointToActuatorTorque",
                                                 robot.NumberOfActuators, robot.NumberOfJoints,
...
bool osaIO1394XMLConfig::ConfigureCouplingMatrix(cmnXMLPath & xmlConfig,
                                                 const int robotIndex,
...
bool osaIO1394XMLConfig::ConfigureDigitalInput(cmnXMLPath & xmlConfig,
                                               const int tagIndex,
                                               osaIO1394::DigitalInputConfiguration & digital_input)
{
